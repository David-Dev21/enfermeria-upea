    ...
    return (
        <div ref={cardRef} className="relative w-full max-w-xl my-auto">
            <div className="relative w-full h-auto">
                <div className={`front absolute inset-0 flex flex-col p-4 rounded-3xl ${frontBg} overflow-hidden min-h-full`}>
                    <h2 className={`font-bold text-xl text-center ${frontTextColor}`}>
                        <FontAwesomeIcon icon={icon} className={frontTextColor} /> {frontTitle}
                    </h2>
                    <div className="mt-4 flex-1 overflow-hidden">
                        {frontContent}
                    </div>
                </div>
                <div className={`back absolute inset-0 flex flex-col items-center p-4 rounded-3xl shadow-md ${backBg} overflow-hidden min-h-full`}>
                    <h2 className={`font-bold text-xl text-center mb-4 ${backTextColor}`}>
                        <FontAwesomeIcon icon={icon} className={backTextColor} /> {backTitle}
                    </h2>
                    <div className={`text-sm flex-1 overflow-auto ${backTextColor}`}>{backContent}</div>
                </div>
            </div>
        </div>
    );
